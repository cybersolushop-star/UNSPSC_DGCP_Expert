...
Versión corregida para manejar correctamente los encabezados del PDF
"""
...
def extraer_datos_pdf_corregido(ruta_pdf):
    """Extrae los datos del PDF de manera corregida"""
    try:
        datos_pdf = []
        with pdfplumber.open(ruta_pdf) as pdf:
            ...
            for pagina in tqdm(pdf.pages, desc="Extrayendo PDF", unit="página"):
                ...
                for tabla in tablas:
                    ...
                    for fila in tabla:
                        ...
                        # Buscar columna de descripción (Descripción Producto)
                        for i, cell in enumerate(fila_str):
                            if 'descripción producto' in cell or 'descripcion producto' in cell:
                                idx_desc = i
                            if 'sinónimos' in cell or 'sinonimos' in cell:
                                idx_sin = i
                        
                        # Si encontramos ambos encabezados, procesamos las siguientes filas
                        if idx_desc is not None and idx_sin is not None:
                            # Buscar datos en las filas siguientes
                            for fila_datos in tabla[tabla.index(fila)+1:]:
                                if fila_datos and len(fila_datos) > max(idx_desc, idx_sin):
                                    ...
                                    if desc and str(desc).strip():
                                        # No es un encabezado ni código vacío
                                        desc_str = str(desc).strip()
                                        if not desc_str.startswith('Código') and not desc_str.startswith('código'):
                                            datos_pdf.append({
                    ...
                    # Si encontramos datos, salimos de la tabla
                    if len(datos_pdf) > 0:
                        break
            ...
            if len(datos_pdf) > 0:
                logger.info("📋 Ejemplos de datos extraídos:")
                for i, item in enumerate(datos_pdf[:3]):
                    logger.info(f"  • {item['descripcion']} → {item['sinonimos'][:50]}...")
...
def mapear_sinonimos(df_excel, datos_pdf):
    ...
    logger.info("📋 Creando mapa de sinónimos...")
    mapa_sinonimos = {}
    ...
    columna_b = df_excel.columns[1]  # Columna "Descripción"
    columna_d = df_excel.columns[3]  # Columna "Sinónimos"
...
def main():
    ...
    print("\n" + "="*60)
    print("📊 MAPEO DE SINÓNIMOS - VERSIÓN CORREGIDA")
    print("="*60)
    ...
    datos_pdf = extraer_datos_pdf_corregido(ruta_pdf)
    if not datos_pdf:
